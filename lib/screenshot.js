...
function takeScreenshot(url, settings, resolver){
  ...
  puppeteer.launch({headless: "new"}).then(async (browser) => {
    ...
    // Add optional page interactions here:

    // await actions(page)

    // End optional interactions

    // Lazy Load option 1 - Full height viewport - does not work with fixed elements
    //let calculatedHeight = await getPageHeight(page)
    //await page.setViewport({ width: viewportWidth, height: calculatedHeight, deviceScaleFactor: pageDPR });

    // Lazy Load option 2 - Scroll full height to kick in lazy load
    await page.evaluate(() => {
